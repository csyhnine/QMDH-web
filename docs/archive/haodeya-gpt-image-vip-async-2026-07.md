...
```json
{
  ...
  "upstream_model_4k": "gpt-image-2-vip",
  "poll_path_template": "/images/generations/{task_id}"
}
```

> 2K/4K 映射请保持与 1K 同名；留空也可（代码默认不再拼 `-2k`/`-4k`）。
...
> **临时方案（2026-07 上游确认）**：2K 仍用同一 `gpt-image-2-vip`，靠 `resolution: "2k"` 出图。  
> **禁止** `model=gpt-image-2-vip-2k`（会 503 ChannelCapability）。网关按 model 名计费，此临时方案仍按 1K VIP 价扣上游费；Studio 侧可继续按 2K 合同价记账。

**4K（未在 Studio 开放，代码预留）：**

同临时方案：默认仍发 `gpt-image-2-vip` + `resolution: "4k"`（勿用 `-4k` 后缀，除非上游另行开通）。
...
| `_resolve_haodeya_async_upstream_model` | 1K/2K/4K → 默认同一 `gpt-image-2-vip`；靠 body `resolution` 区分 |
