...
```json
{
  ...
  "upstream_model_2k": "gpt-image-2-vip-2k",
  "upstream_model_4k": "gpt-image-2-vip-4k",
  "poll_path_template": "/images/generations/{task_id}"
}
```

> 计费已按 model 名对齐（2026-07）：2K 必须发 `gpt-image-2-vip-2k`，勿再用「同名 + 仅 resolution」指望按 2K 价扣费。
...
> **计费对齐（2026-07）**：2K 用 `gpt-image-2-vip-2k` 按次分价。`resolution` / `image_config` 可带可不带（网关会补）。  
> 停用：仅 `gpt-image-2-vip` + `resolution:2k`（仍可能出 2K 图，但按 1K 价扣）。

**4K（未在 Studio 开放，代码预留）：**

默认 `gpt-image-2-vip-4k` + `resolution: "4k"`（若上游已开通对应 SKU）。
...
| Studio 档位 | 实际上游 `model` | body `resolution` |
| --- | --- | --- |
| 1K | `gpt-image-2-vip`（或 Admin「1K 上游模型」） | `1k` |
| 2K | `gpt-image-2-vip-2k`（或 Admin「2K 上游模型」） | `2k` |
| 4K | `gpt-image-2-vip-4k`（或 Admin「4K 上游模型」；Studio UI 暂未开放） | `4k` |

Admin「模型管理」生图渠道可填 1K/2K/4K 上游模型与单价（写入 `adapter_config`）。  
留空时：1K = 基准 `model_name`，2K/4K = `{基准}-2k` / `{基准}-4k`。  
Haodeya 按 **model 名** 扣费；我们日志按 `unit_price_1k/2k/4k` 分档（4K 无价时回退 2K→1K）。

**视频（Grok）**另有四档 SKU 映射（`upstream_sku_i2v_5s` 等），与生图分辨率无关；留空则用 Studio 所选 SKU。

> **历史**：2026-07-03 曾因 `-2k` SKU 未就绪，临时用基座 model + `resolution: 2k`；2026-07-16 起默认拼 `-2k`，并支持 Admin 覆盖。
...
| `_resolve_haodeya_async_upstream_model` | 1K=`gpt-image-2-vip`；2K=`…-2k`；4K=`…-4k`（按 model 名计费） |
...
| 1K/2K 区分 | 同一 model + `image_config.image_size` | **换 `*-2k` model 名**（可兼带 resolution） |
...
| 2K 曾用基座 model | 当时 `-2k` SKU 未就绪 | **已恢复** `gpt-image-2-vip-2k` + `resolution: 2k` |
...
覆盖：1K 提交与轮询、2K → `gpt-image-2-vip-2k`、`result.data[0].url` 解析、参考图 `image_urls`、仅走 `/images/generations/{id}` 轮询、下载 UA。
...
- [ ] Studio 真实联调：1K + 2K（2K 日志 model 应为 `gpt-image-2-vip-2k`；有参考图再测 `image_urls`）
