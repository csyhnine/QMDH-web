...
状态：**本地 WIP，全部未 commit / 未部署生产**  
生产基线仍为 **`main` @ `0090a2a`（v1.1.0）** + 下文「生产热补丁三件套」（与 Git 可能不一致）

> **新对话请先读**：`docs/handoff.md`（最新 3 条）→ 本文 → `docs/tasks.md`（Agent 任务块）→ `docs/product-boundary.md`

---

## 1. 一句话现状

Web Chat（`/studio/chat`）已从「纯 LLM 对话」扩展到 **B1 只读 tools + gov 策略 + 多 Agent LangGraph + HITL 生图确认 + 外网抓取 C1/C2 + ref-intent MVP**；代码量 ~5k+ 行 diff，**仍在工作区未提交**。生产服务器 **没有** 这些 Agent 能力；仅图像执行层有 2026-07-01 热补丁。
