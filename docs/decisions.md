...
- 日期：2026-05-16
- 背景：
  - 项目后续明确存在“研究型 / 协作型智能工作流平台”的 2.0 升级方向
  - 当前 1.0 仍以设计生产平台的稳定化、可运营化和数据保全为主
  - 若现在直接引入通用 Agent 平台或重编排框架，容易把当前版本拖入过大范围并制造新的技术负债
- 决策内容：
  - 2.0 只作为升级方向与路线约束，不作为当前版本的立即交付目标
  - 1.0 后续中大型需求必须考虑 2.0 兼容性，但不要求当前就落地完整智能体系统
  - 优先推进那些既服务 1.0 当前闭环、又能作为 2.0 地基的基础能力
- 影响：
  - 后续需求评审要增加“2.0 Compatibility Check”
  - 数据结构、存储策略、项目沉淀和观测能力要避免把未来多步骤工作流路径封死
