...
from app.models import AgentClient, AgentJob, AgentPersona, AgentPolicyOverride, AgentSkillRelease, Asset, Conversation, InspirationPost, Project, ProjectResearchNote, Task, TaskStatus, User, UserAgentAssignment, Workflow
...
def _to_persona_out(persona: AgentPersona) -> AgentPersonaOut:
    return AgentPersonaOut(
        ...
        chat_tool_allowlist=list(persona.chat_tool_allowlist or []),
...
def _to_user_roster_out(db: Session, user: User) -> UserAgentRosterOut:
    roster = load_user_agent_roster(db, user.id)
    return UserAgentRosterOut(
        user_id=user.id,
        user_name=user.name,
...
@router.get("/admin/personas", response_model=list[AgentPersonaOut])
def list_agent_personas(
    db: Session = Depends(get_db),
    auth_user=Depends(get_current_auth_user),
) -> list[AgentPersonaOut]:
    require_ops_access(auth_user)
    personas = db.scalars(select(AgentPersona).order_by(AgentPersona.sort_order.asc(), AgentPersona.id.asc())).all()
    return [_to_persona_out(item) for item in personas]


@router.post("/admin/personas", response_model=AgentPersonaOut, status_code=status.HTTP_201_CREATED)
def create_agent_persona(
    payload: AgentPersonaCreate,
    db: Session = Depends(get_db),
    auth_user=Depends(get_current_auth_user),
) -> AgentPersonaOut:
    require_ops_access(auth_user)
    existing = db.scalar(select(AgentPersona).where(AgentPersona.key == payload.key.strip()))
    if existing is not None:
        raise HTTPException(status_code=409, detail="Agent persona key already exists")
    ...
    db.commit()
    db.refresh(persona)
    return _to_persona_out(persona)


@router.patch("/admin/personas/{persona_id}", response_model=AgentPersonaOut)
def update_agent_persona(
    ...
    update_data = payload.model_dump(exclude_unset=True)
    if "chat_tool_allowlist" in update_data and update_data["chat_tool_allowlist"] is not None:
        update_data["chat_tool_allowlist"] = list(normalize_chat_tool_allowlist(update_data["chat_tool_allowlist"]))
    for field, value in update_data.items():
...
@router.get("/admin/users/{user_id}/agent-roster", response_model=UserAgentRosterOut)
def get_user_agent_roster(
    user_id: int,
    db: Session = Depends(get_db),
    auth_user=Depends(get_current_auth_user),
) -> UserAgentRosterOut:
    ...
    ensure_default_roster_for_user(db, user_id)
    db.commit()
    return _to_user_roster_out(db, user)


@router.put("/admin/users/{user_id}/agent-roster", response_model=UserAgentRosterOut)
def update_user_agent_roster(
    user_id: int,
    payload: UserAgentRosterUpdate,
    db: Session = Depends(get_db),
    auth_user=Depends(get_current_auth_user),
) -> UserAgentRosterOut:
    require_ops_access(auth_user)
    ...
    try:
        replace_user_roster(
            ...
            primary_persona_id=payload.primary_persona_id,
        )
    except ValueError as exc:
...
@router.post("/admin/users/{user_id}/agent-roster/reset-default", response_model=UserAgentRosterOut)
def reset_user_agent_roster(
    user_id: int,
    db: Session = Depends(get_db),
    auth_user=Depends(get_current_auth_user),
) -> UserAgentRosterOut:
