"""Chat agent write tools: task proposals and confirmed submission (HITL / Phase B2)."""
...
WRITE_TOOL_BY_WORKFLOW = {
    "image-generate": "create_image_generate_task",
    "image-edit": "create_image_edit_task",
    "video-generate": "create_video_generate_task",
}
...
def auth_user_from_tool_context(ctx: StudioToolContext) -> AuthUserProfile:
    user = ctx.db.get(User, ctx.user_id) if ctx.user_id is not None else None
    project_codes = tuple(str(code) for code in (user.project_codes or []) if str(code).strip()) if user else ()
    return AuthUserProfile(
        name=ctx.user_name,
        token="",
        role=(user.role if user is not None else "designer"),
        project_codes=project_codes or ("QMDH-001",),
        user_id=ctx.user_id,
        display_name=((user.display_name if user is not None else "") or ctx.user_name),
    )


def auto_submit_chat_agent_proposal(ctx: StudioToolContext, proposal: ChatAgentTaskProposal) -> dict[str, Any]:
    """Create the Studio task immediately (no HITL confirm step)."""
    from app.services.agent_policy_service import EffectiveChatPolicy, resolve_effective_chat_policy

    auth_user = auth_user_from_tool_context(ctx)
    policy = ctx.policy if isinstance(ctx.policy, EffectiveChatPolicy) else None
    if policy is None:
    ...
    task = submit_confirmed_chat_agent_task(
        ctx.db,
        auth_user=auth_user,
        policy=policy,
        proposal=proposal,
    ...
    data["summary"] = f"已入队 #{task.id} · {proposal.summary}"
    return data
...
def resolve_default_project_code(db: Session, *, user_id: int | None, auth_user: AuthUserProfile | None = None) -> str:
    ...
    project = db.scalar(select(Project).where(Project.archived_at.is_(None)).order_by(Project.id.asc()))
    if project is None:
...
def _task_payload_result_fields(payload: dict[str, Any]) -> dict[str, str]:
    return {
        ...
        "motion_prompt": str(payload.get("motion_prompt") or "").strip(),
...
def resolve_requested_provider_name(db: Session, requested: str, *, capability_prefix: str = "image.") -> str:
    ...
    enabled = [
        name
        for name, definition in provider_map.items()
        if any(capability.startswith(capability_prefix) for capability in definition.capabilities)
    ]
    hint = ", ".join(enabled[:6]) or "none enabled"
    raise ValueError(f"Provider not found: {raw}. Enabled providers: {hint}")
...
def _validate_task_inputs(
    ...
    *,
    ...
    capability_prefix = "video." if workflow_key == "video-generate" else "image."
    provider_map = get_provider_map(db)
    resolved_provider = resolve_requested_provider_name(
        db, requested_provider, capability_prefix=capability_prefix
    )
    ...
    prompt = str(
        payload.get("prompt")
        or payload.get("edit_prompt")
        or payload.get("motion_prompt")
        or ""
    ).strip()
    if not prompt:
...
def build_image_generate_proposal(
    ...
    *,
    ...
    summary = f"{payload['aspect_ratio']} · {payload['resolution'].upper()} · {display_name} · {resolved_project}"
...
def build_video_generate_proposal(
    ctx: StudioToolContext,
    *,
    motion_prompt: str,
    requested_provider: str,
    ...
    if len(cleaned_prompt) < 3:
        raise ValueError("motion_prompt must be at least 3 characters.")

    resolved_project = (project_code or "").strip() or resolve_default_project_code(ctx.db, user_id=ctx.user_id)
    resolved_provider = resolve_requested_provider_name(
        ctx.db, requested_provider, capability_prefix="video."
    )
    source_path = str(source_image or "").strip()
    payload: dict[str, Any] = {
        "motion_prompt": cleaned_prompt,
        "prompt": cleaned_prompt,
        "aspect_ratio": _normalize_aspect_ratio(aspect_ratio),
    }
    if source_path:
        payload["source_images"] = [source_path]
        payload["reference_image"] = source_path
    ...
    task_title = (title or "").strip() or cleaned_prompt[:48]
    if len(task_title) < 3:
        task_title = cleaned_prompt[:48] or "Chat 视频任务"

    summary = f"视频 · {payload['aspect_ratio']} · {display_name} · {resolved_project}"
    return ChatAgentTaskProposal(
        proposal_id=str(uuid.uuid4()),
        workflow_key="video-generate",
        title=task_title[:150],
